...
reserved_word=['mostra', 'enquanto', 'caso', 'seNao', 'funcao', 'chama', 'posicao', 'atirar']
        ...
        if c.isdigit():
            ...
            if self.source[self.position].isdigit():
                ...
                if self.position < len(self.source) and (self.source[self.position].isalpha() or self.source[self.position] == '_'):
                    ...
                    self.next = Token('COORDINATE', value)
            else:
                while self.position < len(self.source) and (self.source[self.position].isalnum() or self.source[self.position] == '_'):
                    value += self.source[self.position]
                    self.position += 1
...
class Shoot(Node):
    ...
    def evaluate(self, st):
        inimigo = st.get_key_by_value(self.value) 
        if inimigo is None:
            raise Exception('Inimigo não encontrado nessa posição')
...
class Parser:
    ...
    def parseStatement(self):
        if self.tokenizer.next.type == 'NEWLINE':
            self.tokenizer.selectNext()
            return NoOp()
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'atirar':
            self.tokenizer.selectNext()
            if self.tokenizer.next.type == 'COORDINATE':
                coord = self.tokenizer.next.value
                self.tokenizer.selectNext()
                return Shoot(coord)
        elif self.tokenizer.next.value == 'chama':
            self.tokenizer.selectNext()
        ...
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'posicao':
            self.tokenizer.selectNext()
            if self.tokenizer.next.type == 'IDEN':
                name = self.tokenizer.next.value
                self.tokenizer.selectNext()
        ...
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'mostra':
            self.tokenizer.selectNext()
            if self.tokenizer.next.type == 'LPAREN':
                self.tokenizer.selectNext()
                ...
                if self.tokenizer.next.type == 'RPAREN':
                    ...
                    raise Exception('Unexpected token: ' + self.tokenizer.next.type)
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'enquanto':
            self.tokenizer.selectNext()
        ...
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'caso':
            self.tokenizer.selectNext()
            condition = self.parseRelExpr()
            if self.tokenizer.next.type == 'LBRACE':
                self.tokenizer.selectNext()
                ...
                if self.tokenizer.next.type == 'RBRACE':
                    self.tokenizer.selectNext()
                if self.tokenizer.next is not None and self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'seNao':
                    self.tokenizer.selectNext()
                    if self.tokenizer.next.type == 'LBRACE':
                        self.tokenizer.selectNext()
        ...
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'funcao':
            self.tokenizer.selectNext()
            name = self.tokenizer.next.value
            self.tokenizer.selectNext()
            if self.tokenizer.next.type == 'LPAREN':
    ...
    def parseFactor(self):
        if self.tokenizer.next.type == "NUMBER":
        ...
        elif self.tokenizer.next.type == 'COORDINATE':
            result = self.tokenizer.next.value
            self.tokenizer.selectNext()
            return coordVal(result)
            ...
            self.tokenizer.selectNext()
            return Iden(result)
        elif self.tokenizer.next.type == 'RESERVED' and self.tokenizer.next.value == 'chama':
            if self.tokenizer.next.type == 'IDEN':
                result = self.tokenizer.next.value
                self.tokenizer.selectNext()
                if self.tokenizer.next.type == 'LPAREN':
